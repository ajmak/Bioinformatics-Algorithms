...
class DirectedAcyclicGraph:
    ...
    def longestPath(self, topOrder, edgeDict, transitionDict, emissionDict):
        ...
        weightDict = {key: {state: float(0) for state in edgeDict.keys() if state != 'source' and state != 'sink'}for key in range(1, len(topOrder)-1)}
        weightDict[0] = {'source': 1}
        weightDict[len(topOrder)-1] = {'sink': float(0)}
        ...
        print(sum(weightDict[len(topOrder)-2].values()))
