...
#                                                                  #
# Runs about 30 seconds faster on largest file                     #
# OO problems fixed                                                #
# Ranges are correct now so won't break on valid min/max           #
# Int to motif indexing fixed                                      #
# min/max edge cases added                                         #
####################################################################
"""
...
class StoreCounts:
    ...
    def computeSeq(self, fastLengths, motifDict):
        ...
        # --maxMotif                                              #
        maxIndex = minIndex = 0
        for i in range(1, self.maxi + 1): maxIndex += 4**i
        for i in range(1, self.mini): minIndex += 4**i
#        if self.mini == 4:
...
def main():
    ...
    # CHANGED all arguments parsed in main instead of parsed in __init__
    mini = arguments.args.minMotif
    maxi = arguments.args.maxMotif
    cutoff = arguments.args.cutoff

    if mini > maxi:
        raise ValueError("Minimum motif length cannot exceed maximum. Exiting")
...
#        CHANGED from: getCounts = StoreCounts(arguments.args, seqs, motifDict, fastLength)
