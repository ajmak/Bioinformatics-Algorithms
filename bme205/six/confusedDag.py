...
class DirectedAcyclicGraph:
    ...
    def longestPath(self, topOrder, edgeDict, transitionDict, emissionDict):
        ...
        pointBack = {}
        weightDict = {key: {state: float('-inf') for state in edgeDict.keys() if state != 'source' and state != 'sink'}for key in range(len(topOrder)-1)}
        weightDict[0]['source'] = 0
        weightDict[len(topOrder)-1] = {'sink': 1}
        print(weightDict)
        print('edgedict')
        print(topOrder)
#        pdb.set_trace()
        for i in range(1, len(topOrder)-1):
#            if i == 0:
#                curState = 'source'

#                continue
#            if i == len(topOrder) - 1:
#                weightDict[i] = weightDict[i-1] + edgeDict['sink'][prevState]
#                break
            print(topOrder[i])
...
#                pdb.set_trace()
                ...
                for parent in weightDict[i-1].keys():
                    ...
                    nodeWeight = float(parent+child) + float(emissionDict[child + topOrder[i]])
                    if weightDict[i][child[0]] < weightDict[i-1][parent] + nodeWeight:
                        print('********')
                        weightDict[i][child[0]] = weightDict[i-1][parent] + nodeWeight
                        pointBack[i] = parent
                        bestChild = child[0]
            curState = bestChild
        print(weightDict)
