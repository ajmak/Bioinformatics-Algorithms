...
class DirectedAcyclicGraph:
    ...
    def longestPath(self, start, end, step, topOrder, edgeDict, emissionDict):
        ...
        weightDict = {key: {state: float(0) for state in edgeDict.keys() if state != 'source' and state != 'sink'}for key in range(start-step, end-step, step)}
        if start == 1:
            weightDict[start-step] = {'source': 1}
            weightDict[end-step] = {'sink': float(0)}
            pointBack[end-step] = {'sink': 'parent'}
        else:
            weightDict[end-step] = {state: float(1.0) for state in edgeDict.keys() if state != 'sink'}
                ...
                for parent in weightDict[i-step].keys():
                    ...
                    if child == 'sink' or parent == 'sink': 
                        ...
                        if len(topOrder) - 2 < start:
                            nodeWeight = float(edgeDict[parent][child]) * float(emissionDict[parent + topOrder[i]])
                            weightDict[i][child] += weightDict[i-step][parent] * nodeWeight
                        else:
                            nodeWeight = float(edgeDict[parent][child]) * float(emissionDict[child + topOrder[i]])
                            weightDict[i][child] += weightDict[i-step][parent] * nodeWeight
        if end == 0: end = 3
