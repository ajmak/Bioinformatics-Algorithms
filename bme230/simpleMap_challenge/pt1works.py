...
class SeedCluster:
    ...
    def clusterSeeds(seeds, l):
        ...
        for i in range(len(seeds)):
            ...
            for y1 in y1s:
                for y1b in y1s:
                    if abs(y1-y1b) <=l and (x1,y1) != (x1,y1b):

                        if (x1,y1) not in edgeDict.keys():
                            edgeDict[(x1,y1)] =[(x1,y1),(x1,y1b)]
                        else:
                            edgeDict[(x1,y1)].append((x1,y1b))
            #for all x,y pairs after to be compared to
            for j in range(i+1, len(seeds)):
                x2, y2s = seeds[j]
                if (abs(x1-x2) <= l):
                    #for all y's in the original x
                    for y1 in y1s:
                        #for all y's in the x being compared
                        for y2 in y2s:
                            print(x1,y1)
                            print(x2,y2)
                            if (abs(y1-y2)) <= l and (x1,y1) != (x2,y2):
                                if (x1,y1) not in edgeDict.keys():
                                    edgeDict[(x1,y1)] = [(x2,y2),(x1,y1)]
                                else:
                                    edgeDict[(x1,y1)].append((x2,y2))
                                if (x2,y2) not in edgeDict.keys():
                                    edgeDict[(x2,y2)] = [(x2,y2),(x1,y1)]
                                else:
